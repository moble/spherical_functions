...
#                                     * sp.Wigner_D_element(0.0, -iota, -phi, ell, -s, m)
#                                     - slow_sYlm(s,ell,m,iota,phi) ) < 1e4 * ell_max**6 * precision_SWSH
            ...
            for ell in range(ell_max+1):
                for s in range(-ell,ell+1):
                    for m in range(-ell,ell+1):
                        ...
                        # print( sp.SWSH(R.a, R.b, s, np.array([[ell,m]]))[0],
                        #        slow_sYlm(s,ell,m,iota,phi) )
                        assert abs( sp.SWSH(R.a, R.b, s, np.array([[ell,m]]))[0]
                                    - slow_sYlm(s,ell,m,iota,phi) ) < 1e4 * ell_max**6 * precision_SWSH
        ...
        for phi in special_angles:
            for ell in range(ell_max+1):
                for s in range(ell):
                    ...
                    LMS = np.array([[ell,-s,m] for m in range(-ell,ell+1)])
