...
def test_modes_addition():
    ...
    # Test failure on different spins
    m2 = sf.Modes(a2, s=s2+1, ell_min=ell_min2, ell_max=ell_max2)
    with pytest.raises(ValueError):
        m1m2 = m1.add(m2)
    # Test success on the same spins
...
def test_modes_conjugate():
    for inplace in [False, True]:
        ...
        assert np.allclose(g, np.conjugate(gbar), rtol=1e-15, atol=1e-15)
