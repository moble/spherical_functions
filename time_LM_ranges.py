...
for i,ell_max in enumerate(ells):
    ...
    result = ipython.magic("timeit -o global LMpM; LMpM = sp.LMpM_range(ell_min, ell_max)")
    picoseconds[i] = 1e12*result.best/evals[i]
    print("With ell_max={0}, and {1} evaluations, each LMpM averages {2:.0f} ps".format(ell_max,evals[i],picoseconds[i]))
    result = ipython.magic("timeit -o global LMpM; LMpM = np.array([[ell,mp,m] for ell in range(ell_min,ell_max+1) for mp in range(-ell,ell+1) for m in range(-ell,ell+1)])")
    picoseconds[i] = 1e12*result.best/evals[i]
    print("\tWith ell_max={0}, and {1} evaluations, each LMpM averages {2:.0f} ps".format(ell_max,evals[i],picoseconds[i]))
    sys.stdout.flush()
